...
class Square:
    ...
    def __init__(self, size=0, position=(0, 0)):
        ...
        Args:
            size: size of new square in int
            position (int, int): new square psn
        """
        self.size = size
    ...
    def area(self):
        """current(instance) area of square is returned."""
    ...
    def __str__(self):
        """Definition of the print() representation of a Square."""
        if self.__size != 0:
            [print("") for i in range(0, self.__position[1])]
        for i in range(0, self.__size):
            [print(" ", end="") for j in range(0, self.__position[0])]
